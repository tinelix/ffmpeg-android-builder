#!/bin/bash
#  Build FFmpeg 0.6.5 for legacy Android devices
#
...
chmod -R 0777 .
mkdir -p ffmpeg-0.6.5/android
mkdir -p ffmpeg-0.6.5/android/armeabi
mkdir -p ffmpeg-0.6.5/android/armeabi-v7a
mkdir -p ffmpeg-0.6.5/android/x86
...
if [ -z "$ANDROID_NDK_HOME" ]; then # requires NDK r7b-r10e
	...
	if [ $FFMPEG_INPUT_ARCH == "armv8a" ]; then
		...
		if [[ $NDK_RELEASE == "r8e" || $NDK_RELEASE == "r7" ]]; then
