...
FFMPEG_VERSION="$(cat ./ffmpeg-2.3/RELEASE)"
...
if [ ! -d "ffmpeg-2.3" ]; then
  echo "[ERROR] FFmpeg 2.3 source code not found. Please download it from https://github.com/ffmpeg/ffmpeg.";
  exit 1
fi

echo "Creating output directories...";
chmod -R 0777 .
mkdir -p ffmpeg-2.3/android
mkdir -p ffmpeg-2.3/android/armeabi
...
if [ -z "$ANDROID_NDK_HOME" ]; then # requires NDK r7b-r10e
	echo "[ERROR] NDK (requires r10e) not installed or ANDROID_NDK_HOME variable is not defined. Quiting...";
	exit 1
else
