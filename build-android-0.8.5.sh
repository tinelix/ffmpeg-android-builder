...
if [ -z "$ANDROID_NDK_HOME" ]; then # requires NDK r7b-r10e
	...
	if [ $FFMPEG_INPUT_ARCH == "armv8a" ]; then
        ...
        if [ $NDK_RELEASE == "r8e" ]; then
			ANDROID_NDK_TOOLCHAINS="${ANDROID_NDK_HOME}/toolchains/${ANDROID_TOOLCHAIN_CPUABI}-4.4.3/prebuilt/${FFMPEG_BUILD_PLATFORM}-x86_64/bin/${ANDROID_TOOLCHAIN_CPUABI}"
			...
			ANDROID_NDK_TOOLCHAINS="${ANDROID_NDK_HOME}/toolchains/${ANDROID_TOOLCHAIN_CPUABI}-4.4.3/prebuilt/${FFMPEG_BUILD_PLATFORM}-x86/bin/${ANDROID_TOOLCHAIN_CPUABI}"
